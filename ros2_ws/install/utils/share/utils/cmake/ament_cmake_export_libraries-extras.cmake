...
set(_exported_libraries "utils__rosidl_generator_c;utils__rosidl_typesupport_c;utils__rosidl_typesupport_cpp")
set(_exported_library_names "")

# populate utils_LIBRARIES
if(NOT _exported_libraries STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^-" AND NOT "${_arg}" MATCHES "^-[l|framework]")
      list(APPEND utils_LIBRARIES "${_arg}")
      math(EXPR _i "${_i} + 1")
      continue()
    endif()
    ...
    if("${_arg}" MATCHES "^(debug|optimized|general)$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'utils' passes the build configuration keyword '${_cfg}' as the last exported library")
      endif()
    ...
    if(NOT IS_ABSOLUTE "${_library}")
      ...
      if(NOT _lib)
        ...
        message(FATAL_ERROR "Package 'utils' exports the library '${_library}' which couldn't be found")
      elseif(NOT IS_ABSOLUTE "${_lib}")
        # the found library must be an absolute path
        message(FATAL_ERROR "Package 'utils' found the library '${_library}' at '${_lib}' which is not an absolute path")
      elseif(NOT EXISTS "${_lib}")
        # the found library must exist
        message(FATAL_ERROR "Package 'utils' found the library '${_lib}' which doesn't exist")
      else()
        list(APPEND utils_LIBRARIES ${_cfg} "${_lib}")
      endif()

    else()
      if(NOT EXISTS "${_library}")
        # the found library must exist
        message(WARNING "Package 'utils' exports the library '${_library}' which doesn't exist")
      else()
        list(APPEND utils_LIBRARIES ${_cfg} "${_library}")
      endif()
    endif()
  endwhile()
endif()
...
# and add the libraries to utils_LIBRARIES
if(NOT _exported_library_names STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^(debug|optimized|general)$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'utils' passes the build configuration keyword '${_cfg}' as the last exported target")
      endif()
    ...
    if(NOT _library_dirs)
      ...
      if(NOT _lib)
        ...
        message(WARNING "Package 'utils' exports library '${_library_name}' which couldn't be found")
      endif()
    else()
      ...
      if(NOT _lib)
        # warn about not existing library and later ignore it
        message(WARNING
          "Package 'utils' exports library '${_library_name}' with LIBRARY_DIRS '${_library_dirs}' which couldn't be found")
      endif()
    endif()
    if(_lib)
      list(APPEND utils_LIBRARIES ${_cfg} "${_lib}")
...
# TODO(dirk-thomas) deduplicate utils_LIBRARIES
