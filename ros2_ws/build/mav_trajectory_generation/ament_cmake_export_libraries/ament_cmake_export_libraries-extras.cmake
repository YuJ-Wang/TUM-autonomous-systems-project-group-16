...
set(_exported_libraries "mav_trajectory_generation")
set(_exported_library_names "")

# populate mav_trajectory_generation_LIBRARIES
if(NOT _exported_libraries STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^-" AND NOT "${_arg}" MATCHES "^-[l|framework]")
      list(APPEND mav_trajectory_generation_LIBRARIES "${_arg}")
      math(EXPR _i "${_i} + 1")
      continue()
    endif()
    ...
    if("${_arg}" MATCHES "^(debug|optimized|general)$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'mav_trajectory_generation' passes the build configuration keyword '${_cfg}' as the last exported library")
      endif()
    ...
    if(NOT IS_ABSOLUTE "${_library}")
      ...
      find_library(
        _lib NAMES "${_library}"
        PATHS "${mav_trajectory_generation_DIR}/../../../lib"
        NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH
      )

      if(NOT _lib)
        # warn about not existing library and ignore it
        message(FATAL_ERROR "Package 'mav_trajectory_generation' exports the library '${_library}' which couldn't be found")
      elseif(NOT IS_ABSOLUTE "${_lib}")
        # the found library must be an absolute path
        message(FATAL_ERROR "Package 'mav_trajectory_generation' found the library '${_library}' at '${_lib}' which is not an absolute path")
      elseif(NOT EXISTS "${_lib}")
        # the found library must exist
        message(FATAL_ERROR "Package 'mav_trajectory_generation' found the library '${_lib}' which doesn't exist")
      else()
        list(APPEND mav_trajectory_generation_LIBRARIES ${_cfg} "${_lib}")
      endif()

    else()
      if(NOT EXISTS "${_library}")
        # the found library must exist
        message(WARNING "Package 'mav_trajectory_generation' exports the library '${_library}' which doesn't exist")
      else()
        list(APPEND mav_trajectory_generation_LIBRARIES ${_cfg} "${_library}")
      endif()
    endif()
  endwhile()
endif()
...
# and add the libraries to mav_trajectory_generation_LIBRARIES
if(NOT _exported_library_names STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^(debug|optimized|general)$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'mav_trajectory_generation' passes the build configuration keyword '${_cfg}' as the last exported target")
      endif()
    ...
    if(NOT _library_dirs)
      ...
      if(NOT _lib)
        ...
        message(WARNING "Package 'mav_trajectory_generation' exports library '${_library_name}' which couldn't be found")
      endif()
    else()
      ...
      if(NOT _lib)
        # warn about not existing library and later ignore it
        message(WARNING
          "Package 'mav_trajectory_generation' exports library '${_library_name}' with LIBRARY_DIRS '${_library_dirs}' which couldn't be found")
      endif()
    endif()
    if(_lib)
      list(APPEND mav_trajectory_generation_LIBRARIES ${_cfg} "${_lib}")
...
# TODO(dirk-thomas) deduplicate mav_trajectory_generation_LIBRARIES
